...
La implementación usa `@excel.js/exceljs` y carga la candidata clonada `templates/approved/solicitud-factura-soprole-clone-v1.xlsx`. Cada exportación abre una copia en memoria, completa sólo celdas existentes del Excel real, usa saltos de línea para múltiples receptores y CP/MS, escribe enteros CLP exactos sin `number`, permite sólo fórmulas controladas de montos en `C16`/`C17`, valida OpenXML y entrega `SOLICITUD_FACTURA_CLONE_CANDIDATE_V1`. La referencia Soprole `.xls` privada no entra al contexto Docker ni se abre en runtime. La descarga continúa devolviendo el BYTEA persistido, por lo que toda versión histórica permanece byte a byte.
